...
sim_forecast <- sim_forecast[,4:ncol(sim_forecast)]
...
choices <- c("Biden", "Trump", "Uncalled")
...
ui <- fluidPage(
  ...
  fluidRow(
    column(
      ...
      # for (i in 1:50){
      #   cat("prettyRadioButtons(inputId = state.abb[",i,"],  label = state.abb[",i,"],
      #                      choices = choices, selected= choices[3],
      #                      shape = \"round\", status = \"primary\",
      #                      fill = TRUE, inline = TRUE),","\n")
      # }
            
      prettyRadioButtons(inputId = state.abb[1],  label = state.name[1],
                          choices = choices, selected= choices[3],
                          shape = "round", status = "primary",
                          fill = TRUE, inline = TRUE),
      prettyRadioButtons(inputId = state.abb[2],  label = state.name[2],
                         choices = choices, selected= choices[3],
                         shape = "round", status = "primary",
                         fill = TRUE, inline = TRUE),
