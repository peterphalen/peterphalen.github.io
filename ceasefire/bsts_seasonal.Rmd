...
transformed parameters {
  vector[n] mu; //Level
  vector[n] mu_err = sigma_level * mu_z;
...
model {
  ...
  y ~ poisson_log(mu + y_seasonal + w_seasonal);
}"
...
```{r}
fit <- stan(model_code=model_code, data=stan_data, cores=2, chains=2, iter = 50, control=list(adapt_delta=.95, max_treedepth=20))
```
