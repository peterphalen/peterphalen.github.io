---
title: Specific social supports and social coping behaviors may prevent suicidal ideation in male veteran sample with mental health difficulties
author: "peter phalen"
date: "4/10/2019"
...
We first run correlations between all the COPE subscales and suicide ideation (```BSS_log```). Darker red indicates a more positive correlation. Darker blue indicates more negative.
...
#### Filtering on statistical significance

Now we want to make a version that filters for significance. To do this, we need to define a function that calculates p-values.
```{r}
...
And we use it to make our plot. Nonsignificant correlations (p>.05) how have an X drawn over them.
```{r}
 ...
 * I think that my friends feel that I’m not very good at helping them solve their problems. (reverse-coded)
...
Because model coefficients can be hard to interpret, here we create a plot of self-esteem interpersonal support and suicidality, with the model prediction drawn as a line. Remember, this line represents the relationship between self-esteem social support and suicidality after controlling for other variables in the model constant, including depression.
...
* I feel like I’m not always included by my circle of friends. (reverse coded)
...
* I don’t often get invited to do things with others. (reverse coded)
...
The ISEL Tangible subscale is intended to capture "perceived availability of material aid." If anything it predicts *increased* risk of suicidal ideation in our sample, after controlling for the other variables in the model. (If you remove the other COPE and ISEL variables from the model, ISEL-Tangible predicts decreased suicidality as you would expect.) The Tangible subscale consists of the following 10 items:
...
These are the specific kinds of coping strategies that may be associated with decreased suicidality *even after controlling for depression and anxiety.*
...
p.values <- data.frame(empty.factor = "0", p.values = 1:length(t.tests))
...
ggplot(p.values) + 
  ...
  ggtitle("p-values for t-tests, plotted as points", subtitle = "p=.05 cutoff marked with vertical red line")
```
