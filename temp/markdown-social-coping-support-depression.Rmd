...
We might suspect that the problem is multicollinearity, as (e.g.) ISEL-Tangible and ISEL-Belonging are strongly correlated (see correlation matrix above). We can actually check this by analyzing the posterior samples.

```{r, warnings=FALSE}
ggpairs(as.data.frame(fit, regex_pars = c("ISEL.tangible", "ISEL.belonging", "ISEL.selfesteem", "ISEL.appraisal", "bai_total", "FCESD", "COPE.emotional.social.support")),
        lower=list(continuous=wrap("points", col=rgb(0,0,0.2,alpha=.2))))
```

So multicollinearity doesn't seem to be a big problem for *estimation.* But we can see a negative correlation between the coefficient estimates for ISEL-Tangible and the estimates for ISEL-Belonging and ISEL-Appraisal, which means that attributing greater increases in suicidality to ISEL-Belonging or ISEL-Appraisal requires attributing lesser increases to ISEL-Tangible. 

If we remove these two variables from the model, ISEL-Tangible no longer substantially predicts increased suicidal ideation.  

```{r}
fit.less <- stan_lmer(BSS_log ~
                   (1 | marital) +
                   (1 | race ) +
                   # z-score the continuous predictors to put them on a common scale
                   scale(yr_edu) +
                 ...
                 cores=4,
                 data=dat)
plot(fit.less, regex_pars="tangible")
```

It's still a little unexpected that tbIf we look at the pairs plot again, we can see a

```{r}
ggpairs(as.data.frame(fit.less, regex_pars = c("ISEL.tangible","bai_total", "ISEL.selfesteem", "FCESD", "COPE.emotional.social.support")),
        lower=list(continuous=wrap("points", col=rgb(0,0,0.2,alpha=.2))))
```

```{r}
fit.even.less <- stan_lmer(BSS_log ~
                   (1 | marital) +
                   (1 | race ) +
                   # z-score the continuous predictors to put them on a common scale
                   scale(yr_edu) +
                 ...
                 cores=4,
                 data=dat)
plot(fit.even.less, regex_pars="tangible")
```



This gives us a better way of understanding the negative relationship between ISEL-Tangible and SI in the multivariate model:

```{r}
ggplot(dat) + 
  aes(x = ISEL.selfesteem, y = ISEL.tangible) + 
  geom_point() + 
...
ggplot(p.values) + 
  ...
  ggtitle("p-values for six t-tests, plotted as points", subtitle = ".05 cutoff marked with vertical red line")
```
